...
import PopupCustomerCateringOrderHistory from "./PopupCustomerCateringOrderHistory";

const CustomerCateringOrderHistory = () => {
      ...
      if (currentUser) {
        try {
          ...
          const response = await axios.get(
            `${process.env.REACT_APP_BACKEND_URL}/getCustomerCateringOrdersHistory?email=${currentUser?.email}`,
            {
              headers: {
  ...
  const handleCustomerTranId = async (tran_id) => {
    try {
      setLoading(true);
      const response = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/getCustomerCateringOrdersDetailsHistory?tran_id=${tran_id}`
      );
      ...
      <div id="layoutSidenav">
        ...
        <div id="layoutSidenav_content">
          <main className="customer_order">
                ...
                <tbody className="table-group-divider customer_order_table">
                  {allCustomerOders.map((allCustomerOrder, index) => (
                    <tr key={allCustomerOrder?._id}>
                      ...
                      <td>{allCustomerOrder?.cus_name}</td>

                      <td>{allCustomerOrder?.cus_phone}</td>
            ...
            <PopupCustomerCateringOrderHistory
...
export default CustomerCateringOrderHistory;
